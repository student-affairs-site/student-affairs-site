...
const authenticateTokenMiddleware = (req: CustomRequest, res: Response, next: NextFunction) => {

    let token = req.headers.authorization;

    if (!token || !token.startsWith('Bearer ')) throw new BadRequest('No access token provided');
